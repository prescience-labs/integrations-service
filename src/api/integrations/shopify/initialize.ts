...
export const refreshProducts = async (params: IInitializeStore) => {
  ...
  products.map(async product => {
    const updatedProduct = await DB.Models.Order.findOneAndUpdate({ shopName: params.shopName, orderId: product.id }, Product.serializeFromShopify(product, params.shopName))
